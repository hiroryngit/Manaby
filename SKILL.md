...
name: pen-design
description: >
  Create high-quality visual designs 窶� websites, app screens, dashboards, slides, marketing materials, social media graphics 窶� using the pen.dev CLI tool. Use this skill whenever the user wants to create, generate, or visualize any kind of UI design, mockup, wireframe, layout, webpage, app screen, presentation slide, poster, banner, or marketing asset. Also use it when the user says things like "design me a...", "make a visual for...", "create a mockup of...", "what would X look like?", or wants to turn an idea into a visual. Even if the user doesn't mention "pen.dev" or "design tool" explicitly 窶� if they want something visual created, this is the skill to use.
---
...
**If you don窶冲 know where skills live on this machine**

Agents don窶冲 always get the skills directory from context. When the path isn窶冲 obvious:

- **Ask the user** where their agent or IDE loads skills from, or where they want this skill installed.
- **Check the product窶冱 docs** for 窶徭kills窶�, 窶彗gent skills窶�, or 窶徘lugins窶� 窶� paths differ by tool and version.
...
**Typical skill locations** (confirm with your tool窶冱 current docs 窶� layouts change):
...
| **OpenClaw** | Often `~/.openclaw/skills/`, workspace `.agents/skills/`, or paths in [OpenClaw skills docs](https://docs.openclaw.ai/skills/) 窶� verify for the user窶冱 setup |
...
- **Early in the session**, before the first pen.dev design run (compare `npm view @pen.dev/cli version` to the installed CLI), so you aren窶冲 following stale instructions.
- **Again** if the user says they upgraded the CLI, or if behavior doesn窶冲 match this doc (flags, auth, timing).
- **Not** before every single command 窶� once per session is enough unless something changed or errors suggest a version mismatch.
...
Key flags:
- `--out, -o` 窶� where to save the `.pen` file (required)
- `--prompt, -p` 窶� what to design (required)
- `--prompt-file, -f` 窶� attach an image or text file to send with the prompt (repeatable). Same idea as attaching reference images in the pen.dev editor chat; not for loading the prompt text from a file.
- `--export, -e` 窶� export an image of the result
- `--export-scale` 窶� image resolution multiplier (use 2 for crisp output)
- `--export-type` 窶� format: `png` (default), `jpeg`, `webp`, `pdf`
- `--in, -i` 窶� start from an existing `.pen` file (for iteration)
- `--model, -m` 窶� Claude model to use (defaults to Opus)

### Passing the Prompt

Pass the user's request directly as the prompt 窶� do not expand, or add detail beyond what the user actually said. The pen.dev CLI has its own AI designer agent that handles creative decisions like layout structure, color palettes, typography, spacing, and content. Adding your own design specifics on top of the user's request will conflict with the CLI agent's own judgment and produce worse results.

If the user says "make me a landing page for a coffee shop", the prompt should be exactly that 窶� not a paragraph with hero sections, color palettes, and font choices you invented.

### Timing Expectations

Design generation is not instant 窶� the CLI runs an AI agent that plans the layout, creates each element, and validates the result visually. Expect:
...
Then use the Read tool on the exported PNG 窶� it will render visually since you're a multimodal model.

Always show the image to the user after creating it. This is the whole point 窶� they want to see the visual.
...
- `design.pen` 竊� `design-v2.pen` 竊� `design-v3.pen`
- Or use a single file: `--in design.pen --out design.pen` (overwrites)

## Working Directory

Save design files in the user's current working directory or a subdirectory like `designs/`. Don't use temp directories 窶� the user will want to find and iterate on these files later.
