...
| 項目 | 内容 |
| :--- | :--- |
| 認証方式 | Google アカウント（OAuth 2.0 + PKCE）。ID トークンはサーバー側で署名検証する |
| ロール | `parent` / `student` / `tutor` / `admin` の 4 種 |
| 受け入れ基準 | ロールごとに到達できる画面が制御されていること／セッションがアプリ再起動後も保持されること |

**初回ログインは 3 段階です。** 立場を選び、プロフィールを入力し、確認してから登録します。

```
Googleログイン → ① 立場を選ぶ → ② プロフィール入力 → ③ 確認 → 登録
                       └─ 管理者 → 合言葉 ────────────────→ 登録（②③を通らない）
```

**立場は①でしか決められず、以後変更できません。** そのため③で確認を挟みます。

②で聞くのは**他の画面が実際に読む項目だけ**です。空のまま通すと、講師一覧に担当科目の無い講師が並び、学習カルテに学年の無い生徒が出ます。

| 立場 | 必須項目 | 任意 | どこに出るか |
| :--- | :--- | :--- | :--- |
| 保護者 | 氏名、電話番号 | — | 連絡先として管理者・講師が参照 |
| 生徒 | 氏名、学年 | 学校名 | ダッシュボード、学習カルテ、講師の担当生徒一覧 |
| 講師 | 氏名、担当科目、経歴・自己紹介、指導方針 | — | 講師一覧（F-02）、講師プロフィール |

**管理者だけはプロフィールを入力しません。** 講師一覧にも学習カルテにも現れないため、埋めるべき項目がそもそも無いからです。合言葉が通った時点でアカウントが作られ、管理画面に入ります。
